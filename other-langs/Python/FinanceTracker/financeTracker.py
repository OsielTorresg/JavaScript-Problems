...
def loadFile(file, cfg):
    ...
    with open(file, mode='r') as csv_file:
        ...
        for row in csv_reader:
            ...
            bank = cfg["cfg"]
            transaction = ((txtDate, date, name, amount,
                           category, rounded, bank, wksName))
...
def loadCSV(rows, cfg={"txtDate": 0, "date": 1, "name": 2, "amount": 3, "category": 5, "rAmount": 4, "cfg": 6}):
    for row in rows:
        print([row[cfg["txtDate"]], row[cfg["date"]], row[cfg["name"]], row[cfg["amount"]],
              row[cfg["category"]], row[cfg["rAmount"]], row[cfg["cfg"]]], 2)
        time.sleep(2)
    ...
    for file in pendingFiles:
        for bank in banks:
            if bank == "bofa":
                ...
                if bofa:
                    rows = loadFile(
                        file, {"category": 1, "amount": 4, "name": 2, "date": 0, "cfg": "bofa"})
                    loadCSV(rows)
                    os.remove(file)
            if bank == "apple":
            ...
            if bank == "discover":
                ...
                if discover:
                    ...
                    loadCSV(rows, {"txtDate": 0, "date": 1, "name": 2,
                            "amount": 3, "rAmount": 4, "category": 5, "cfg": 6})
                    os.remove(file)
